...
export default function ChartsSection({ expenses = [] }) {
  ...
  const chartContent = useMemo(() => {
    ...
    const barChartExpenses = selectedMonth === -1
      ? filteredExpenses
      : filteredExpenses
          .filter(e => new Date(e.date).getMonth() === selectedMonth)
          .map(e => ({
            ...e,
            subcategory: typeof e.subcategory === 'object' 
              ? e.subcategory.name 
              : e.subcategory,
          }));
  ...
  ), [year, category, categories, monthNumbers, months, selectedMonth, handleYearChange, handleCategoryChange, isLoading]);
